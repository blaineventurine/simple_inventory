...
class TestBaseServiceHandler:
    ...
    @pytest.mark.asyncio
    async def test_save_and_log_success(self, base_service_handler, mock_coordinator, caplog):
    ...
    @pytest.mark.asyncio
    async def test_save_and_log_success_with_special_characters(self, base_service_handler, mock_coordinator, caplog):
        """Test saving and logging with special characters in names."""
        with caplog.at_level(logging.INFO):
            await base_service_handler._save_and_log_success("my-pantry_01", "Updated item", "café-latte")

        mock_coordinator.async_save_data.assert_called_once_with(
            "my-pantry_01")
        assert "Updated item: café-latte in inventory: my-pantry_01" in caplog.text

    @pytest.mark.asyncio
    async def test_save_and_log_success_coordinator_exception(self, base_service_handler, mock_coordinator):
    ...
    def test_get_inventory_and_name_basic(self, base_service_handler, basic_service_call):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_save_operations(self, base_service_handler, mock_coordinator):
