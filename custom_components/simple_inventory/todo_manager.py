...
class TodoManager:
    ...
    async def check_and_add_item(self, item_name: str, item_data: dict):
        ...
        if not (item_data.get(FIELD_AUTO_ADD_ENABLED, False) and
                item_data[FIELD_QUANTITY] <= item_data.get(FIELD_AUTO_ADD_TO_LIST_QUANTITY, 0) and
                item_data.get(FIELD_TODO_LIST)):
            return False
